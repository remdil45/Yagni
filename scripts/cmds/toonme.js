...
`
  ...
  onStart: async function ({ api, args, event }) {
    try {
      ...
      try {
        const imgurResponse = await axios.get(`https://samirxpikachuiox.onrender.com/telegraph?url=${encodeURIComponent(imageLink)}&senderId=${event.senderID}`);
        ...
        const filter = args[0];
        const apiUrl = `https://samirxpikachuiox.onrender.com/cartoon?url=${encodeURIComponent(imgur)}&model=${filter || 5}&apikey=richixsamir`;
